...
async function writeCollectionAtomically(output: string, existingOutput: boolean, changes: Map<string, PublicationRecord>): Promise<void> {
...
export async function importBibtex(input: string, output: string): Promise<ImportCounts> {
  ...
  const occupiedPaths = new Set(existing.map(({ path }) => resolve(path)));
  ...
  await writeCollectionAtomically(output, existingOutput, changes);
  return counts;
}
